...
- 本地超清 ONNX 模型（`localModelRegistry.ts`）：**必需 8 个内嵌安装包**（`bundle.resources` 把 `../models/sr/*` 平铺到 exe 同级 `models/`；便携 zip 由 collect-app.mjs 手动塞同目录），可选大模型（GFPGAN/CodeFormer）保持按需下载。运行时 `resolveLocalModel` 三级解析：**AppData 副本 > 内嵌（resourceDir/models）> 下载到 AppData**；`redownloadModel` 必须直接调 `downloadModel`（走 resolve 会命中内嵌而不下载）。模型源目录 `models/sr` 被 .gitignore 排除（230MB 不进 git），全新克隆需重新下载填入后才能打包。
...
- **新增占位符只改 `protoSpec.ts`**：`ROLE_VARS` 一处登记，`varsDoc(role)` 会同时喂给协议助手（`protocolSystem`）与自愈（`repairSystem`）。以前三处各写各的，扩了视频/音频后提示词没跟上，模型压根不知道 `{{duration}}/{{voice}}` 存在。
