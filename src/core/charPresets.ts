...
export const CHAR_DELIVERABLES: { value: CharDeliverable; label: string; desc: string }[] = [
  ...
  { value: "closeup", label: "特写三视图", desc: "左：面部及上身近景特写；右：全身正/侧/背三视图（含服装）" },
...
const DELIV_SPEC: Record<CharDeliverable, string> = {
  ...
  closeup:
    "画面左右分两部分排版：左侧为同一角色的面部及上半身近景特写，表情、神态贴合角色气质与参考素材（参考图/文字描述里是什么神态就保持什么神态，不另行指定），面部细节清晰、五官精致，占画面约一半宽度；右侧为同一角色的全身三视图——正面、侧面、背面三个全身站立视图一排排列，每个视图从头到脚完整入镜，发型、五官、服装、鞋帽、配饰与左侧特写完全一致。整体为角色设定参考版式，纯白背景，均匀柔光",
...
所有提示词必须：以同一角色为主体，把发型发色、脸部特征、体型、服装、配饰等外貌锚点完整重复写进每一段（保证多张图角色一致）；用户文字描述里提到的每个设定细节（发色、服装、配饰、特征等）都必须逐字体现进每段提示词，不得遗漏；三视图（turnaround）、特写三视图（closeup 的右侧三视图）与角色立绘（portrait）必须是完整全身画面（若参考图仅为半身，提示词里要明确要求扩展补全全身）；画风与原图/描述一致（写实照片保持写实摄影质感，插画保持同风格插画；文字描述未指明画风时默认写实摄影）；${langLine}。
